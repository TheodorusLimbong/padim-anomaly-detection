...
def print_table(padim_metrics, knn_metrics):
    ...
    print("=" * 60)
    print(f"{'Metric':<20} {'PaDiM':<15} {'KNN (1-NN)':<15}")
    print(sep)
...
def run():
    ...
    # Apply same dim indices for PaDiM
    ...
    print("[INFO] Running KNN (1-NN) inference...")
    knn_scores, knn_maps = compute_knn_scores(
        test_patches_knn, feature_bank, k=1, img_size=IMAGE_SIZE,
    )
