# Cara jalankan: streamlit run dashboard/app.py
# Live inference dashboard: upload gambar -> PaDiM + KNN -> anomaly map -> prediksi ANOMALI/NORMAL

# Cara jalankan: streamlit run dashboard/app.py
# Dashboard interaktif: upload gambar -> inferensi PaDiM + KNN langsung
#
# Fitur:
# - Upload gambar (multi-file)
# - PaDiM anomaly map + score + prediksi ANOMALI/NORMAL
# - KNN anomaly map + score + prediksi ANOMALI/NORMAL
# - Sequential display: feature extraction -> PaDiM -> KNN (PaDiM tampil dulu)
# - Global normalization untuk PaDiM maps (dari experiment data)
# - Metrics: AUROC + PRO-score dari experiment
...
# Cache: backbone + experiment data (di-load sekali)
@st.cache_resource
def get_model(_dev):
...
if uploaded_files:
    for i, uploaded in enumerate(uploaded_files):
        ...
        # Kolom 1: Original image
        with col1:
            st.image(pil_image, caption="Original", use_container_width=True)

        # Kolom 2: PaDiM (tampil duluan karena lebih cepat)
        with col2:
            ...
            if pmax is not None and pmax > pmin:
                map_norm = (map_raw - pmin) / (pmax - pmin)  # global normalization
            else:
                map_norm = normalize_map(map_raw)            # per-image fallback
            ...
            st.caption(f"Time: {padim['time']:.3f}s")

        # Kolom 3: KNN (lebih lambat, muncul setelah PaDiM)
        with col3:
